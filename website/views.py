...
def index2(request):  
	...
	cursor = connections['sqlite'].cursor()
	for row in cursor.execute('SELECT t1.title,t1.slug,t1.html,t2.name,t2.image,DATE(t1.created_at) FROM posts t1 LEFT JOIN users t2 on t2.id = t1.published_by WHERE published_by IS NOT NULL ORDER BY published_at'):
